...
fac_q = rebin(fac_q, n_elements(j_q[*,0,0]), n_elements(fac_q[*,0]), n_elements(fac_q[0,*]))
fac_g = rebin(fac_g, n_elements(j_g[*,0,0]), n_elements(fac_g[*,0]), n_elements(fac_g[0,*]))
;;;; beta value
...
if n_elements(x_check) lt 2 then begin
	...
	if n_outpsbl_q gt 0 then begin
		...
		fac_q_regular[i_outpsbl_q] = !values.f_nan
	...
	if n_outpsbl_g gt 0 then begin
		...
		fac_g_regular[i_outpsbl_g] = !values.f_nan
